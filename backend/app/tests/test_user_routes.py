...
def test_get_user_200(auth_client):
    ...
    created = auth_client.post("/api/users", json=payload).get_json()["data"]
    uid = created["id"]
...
def test_create_user_duplicate_email_400(client):
    ...
    assert "duplicate" in err["message"].lower() or "exists" in err["message"].lower()